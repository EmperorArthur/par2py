...
class FileVerificationPacket(Packet):
    ...
    _single_format = "<" + MD5_FORMAT  # + Variable length data
    ...
    @property
    def id(self) -> bytes:
        """ The File ID (as generated by the FileDescriptionPacket """
        return self._raw_struct[0]

    @property
    def checksums(self) -> Tuple[Tuple[bytes]]:
        """
        The MD5 Hash and crc of each "block" of the file
        """
        offset = struct.calcsize(self._single_format)
        return tuple(struct.iter_unpack(MD5_FORMAT + "4s", self._data_after_header[offset:]))
